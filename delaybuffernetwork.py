...
class DelayBufferNetwork():
    ...
    def build_event_arrays(self):
        ...
        for event_id, event_dict in self.event_dict.items():
            ...
            for agent in agents_at_event:
                self.event_agent_array[int(event_id), int(agent)] = True
    ...
    def get_agent_delay(self):
        ...
        """
        time_agent_delays = np.zeros((self.total_events, 1 + len(self.agent_delays[0,:])))
        for i in range(self.total_events):
    ...
    def plot_delay_propagation(self):
        ...
        time_agent_delays = self.get_agent_delay()
        plt.figure(figsize=(10,10))
        if self.system!="df":
            plt.plot(time_agent_delays[:,0], time_agent_delays[:,1:].mean(axis=1), label="Average agent delay")
        else:
